...
// src/autoembed/index.js
var import_cheerio_without_node_native = __toESM(require("cheerio-without-node-native"));

// src/autoembed/constants.js
var AUTOEMBED_API = "https://player.autoembed.app";
var HEADERS = {
  "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36",
  "Referer": "https://player.autoembed.app/"
};
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      const url = isMovie ? `${AUTOEMBED_API}/embed/movie/${imdbId}` : `${AUTOEMBED_API}/embed/tv/${imdbId}/${season}/${episode}`;
      console.log(`[AutoEmbed] Fetching embed page: ${url}`);
      const resp = yield fetch(url, { headers: HEADERS });
      ...
      const streams = yield extractFromEmbed(embedUrl);
  ...
  return __async(this, null, function* () {
    try {
      const resp = yield fetch(embedUrl, {
        headers: __spreadProps(__spreadValues({}, HEADERS), { "Referer": AUTOEMBED_API + "/" })
      });
